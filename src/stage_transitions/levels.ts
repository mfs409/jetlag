import { Actor, BoxBody, CircleBody, Destination, Enemy, FilledBox, Goodie, Hero, ImageSprite, KeyCodes, Obstacle, Path, PathMovement, Scene, Sensor, TextSprite, TiltMovement, b2Vec2, stage } from "../jetlag";
import { splashBuilder } from "./splash";
import { chooserBuilder } from "./chooser";
import { drawMuteButton } from "./common";

/**
 * levelBuilder is for drawing the playable levels of the game
 *
 ...
 */
export function levelBuilder(level: number) {
  ...
  if (level != 9) {
    stage.score.onLose = { level: level, builder: levelBuilder };
    stage.score.onWin = { level: level + 1, builder: levelBuilder };
  }
  else {
    stage.score.onLose = { level: level, builder: levelBuilder };
