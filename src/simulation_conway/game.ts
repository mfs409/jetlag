import { Actor, BoxBody, FilledBox, GridSystem, JetLagGameConfig, TextSprite, TimedEvent, initializeAndLaunch, stage } from "../jetlag";
...
class Config implements JetLagGameConfig {
  // We cram more meters into the world here, by using fewer pixels per meter.
  // But note that since this is 5x the density of most of our games, we're
  // going to need to scale the fonts by 5x
  aspectRatio = { width: 80, height: 45 };
  hitBoxes = false;
}
...
function builder(level: number) {
  ...
  GridSystem.makeGrid(stage.world, { x: 0, y: 0 }, { x: COLS, y: ROWS }, 2);
  ...
  new Actor({
    ...
    appearance: new TextSprite({ center: true, face: "Arial", size: 36 * 5, color: "#000000" }, () => sim_running ? "Running" : "Paused"),
    gestures: { tap: () => { sim_running = !sim_running; return true; } }
  });

  new Actor({
    rigidBody: new BoxBody({ cx: 18, cy: 42.5, width: 10, height: 4 }, { scene: stage.hud }),
    appearance: new TextSprite({ center: true, face: "Arial", size: 36 * 5, color: "#000000" }, () => mod_mode ? "wrap: ON" : "wrap: OFF"),
  ...
  new Actor({
    ...
    appearance: new TextSprite({ center: true, face: "Arial", size: 36 * 5, color: "#000000" }, () => "Speed: " + (SPEED_MAX - speed_limit)),
  ...
  new Actor({
    ...
    appearance: new TextSprite({ center: true, face: "Arial", size: 36 * 5, color: "#000000" }, () => "Level: " + level),
  ...
  new Actor({
    ...
    appearance: new TextSprite({ center: false, face: "Arial", size: 36 * 5, color: "#000000" }, () => names[level - 1]),
  });
